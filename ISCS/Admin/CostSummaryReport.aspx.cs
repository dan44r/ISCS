...
{
    public partial class CostSummaryReport : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                strOrderNo = Request.QueryString["on"].ToString();
                BindGrid1();
                PopulateReport();
            }
        ...
        protected void btnEmail_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (ViewState["thisDT1"] != null)
                { strCostSummary = Convert.ToString(ViewState["thisDT1"]); }
                GenerateSendMail(strCostSummary);
            }
            catch (Exception ex) { }
        }
        ...
        public void GenerateSendMail(string strBody)
        {
            ...
            int iStat = CommonBL.sendMailWithAttch("ops@3plintegration.com", email, company + " Cost Summary By Order Number Report", sbMailBody1.ToString(), Server.MapPath("../QBFiles/Reports/" + fname));
            if (iStat > 0)
            {
                ScriptManager.RegisterStartupScript(this.Page, this.GetType(), "dfgdfg", "alert('This report has been sent to Email address " + email.ToString() + "');", true);
            }
            else
            {
                ScriptManager.RegisterStartupScript(this.Page, this.GetType(), "hjhj", "alert('Sorry, please try again.');", true);
            }
